...
private:
	...
	//独立轮廓个数
	int sections; 
	//以轮廓上不同点为中心的区域个数（即轮廓上点的个数）区域是因为每个轮廓上点会以stripwidth为半径，向两边扩展
	int areaCount;
	//轮廓
	Contour contour; 
	//TU  坐标值为key，value为inf_point
	Strip strip; 
	int rows, cols;
	//使用DP算法时存储中间值
