...
void BorderMatting::StripInit(const Mat& _mask){
	...
	//宽搜遍历TU  bfs 从轮廓出发，遍历TU  
	// C +- stripwidth 作为TU的范围
	// 索引idex 轮廓上周围邻近点共享相同索引index
	int l = 0;
	while (l < queue.size()){
		point p = queue[l++]; //取出点
		...
		for(int i = 0; i < rstep; i++)	{
			...
			//signed distance from pixel n to contour C  背景是负距离，前景是正距离
...
inline double Gaussian(double _x, double _delta, double _sigma){
	...
	//公式 1/(sigma*sqrt(2*PI)) * exp(-(x-delta)^2/(2*sigma^2))
...
double BorderMatting::dataTerm(int _index, point _p, double _uf, double _ub, double _cf, double _cb, double _delta, double _sigma, const Mat& _gray){
	...
	//宽搜遍历以p为中心点的区域  即以p为中心,半径为stripwidth的区域
	int l = 0;
	while (l < queue.size())
	{
...
void calMeanAndCov(point _p, const Mat& _gray, const Mat& _mask, double& _uf, double& _ub, double& _cf, double& _cb){
	int len = L;//区域半径
	double sumf = 0, sumb = 0;//前景和背景的像素点灰度值之和
	int cntf = 0, cntb = 0;//前景和背景的像素点个数
	...
	for(int x = _p.x - len; x <= _p.x + len; x++)
		for(int y = _p.y - len; y <= _p.y + len; y++)
			if(!(outrange(x, 0, rows - 1) || outrange(y, 0, cols - 1))){
				...
				//背景
				if((_mask.at<uchar>(x, y) & 1) == 0)
					_cb += pow(g - _ub, 2.0);//像素值与背景均值 _ub 的差平方和，加到 _cb。
				//前景
				else _cf += pow(g - _uf, 2.0);
			}
	_cf /= (double)cntf; //前景方差  方差公式
...
void BorderMatting::EnergyMinimization(const Mat& _oriImg, const Mat& _mask){
	...
	for(int i = 0; i < contour.size(); i++){
		...
		for(int d0 = 0; d0 < deltaLevels; d0++) //枚举delta
			for(int s0 = 0; s0 < sigmaLevels; s0++){ //枚举sigma
				...
				//计算term D   轮廓上pp及其周围index相同的邻近点的D之和
				...
				for(int d1 = 0; d1 < deltaLevels; d1++)//枚举index-1时的delta
					for(int s1 = 0; s1 < sigmaLevels; s1++){//枚举index-1时的sigma
						...
						if(rs < ef[index][d0][s0]){
							...
							ef[index][d0][s0] = rs;
							rec[index][d0][s0] = ds;//记录前一个点的delta和sigma
						}
					}
			}
	}
	...
	//采用回溯思想，从最后一个区域开始，一直到第一个区域
	vecds[areaCount - 1] = ds;
	for(int i = areaCount - 2; i >= 0; i--){
		dands ds0 = vecds[i + 1];//当前区域i+1的最优参数
		dands ds = rec[i + 1][ds0.delta][ds0.sigma];//当前区域i的最优参数
		vecds[i] = ds;
	}
}
...
/*	利用bfs计算所有alpha的值  
*	先在轮廓上计算alpha，然后宽搜遍历TU，计算alpha
*	虽然同一个t区域的delta,sigma值相同，但是alpha值不同，因为dis不一样
*	alpha: sigmoid((dis-delta)/sigma)
*/
void BorderMatting::CalculateMask(Mat& _alphaMask, const Mat& _mask){
	...
	for(int i = 0; i < _mask.rows; i++)
		for(int j = 0; j < _mask.cols; j++)
			_alphaMask.at<float>(i, j) = (float)(_mask.at<uchar>(i, j) & 1);
...
void BorderMatting::display(const Mat& _originImage, const Mat& _alphaMask){
	...
	//分离前景和背景通道，设背景色为黑色
	Mat img;
	//缩放至0-1之间  为了让alpha可以在边界区域影响
	_originImage.convertTo(img, CV_32FC3, 1.0 / 255.0);
	cv::split(img, ch_img);//分离三通道 ch_img[0] ch_img[1] ch_img[2]

	//根据alpha的值计算加上透明度以后的图像
	ch_img[0] = ch_img[0].mul(_alphaMask) ;
	ch_img[1] = ch_img[1].mul(_alphaMask) ;
	ch_img[2] = ch_img[2].mul(_alphaMask) ;
	//合并三通道
	Mat res;
	cv::merge(ch_img, res);
	//显示结果
